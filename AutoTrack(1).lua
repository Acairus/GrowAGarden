...
local UNREACHABLE_HEIGHT    = 12   -- studs above you before target is "too high"
local UNREACHABLE_HORIZ     = 6
...
local autoJuke        = false  -- Auto Juke toggle
local autoJukeConn    = nil
local AUTO_JUKE_DISTANCE = 20  -- studs: only juke when bomb carrier is within this range
...
local function startTracking()
	...
	trackConn = RunService.Heartbeat:Connect(function()
		...
		-- Reachability check:
		-- If the target is significantly higher than us AND far enough
		-- horizontally that we can't just jump to them, consider them
		-- unreachable — stop MoveTo and let the player navigate manually.
		local dy         = tRoot.Position.Y - root.Position.Y
		local horizDist  = Vector2.new(tRoot.Position.X - root.Position.X, tRoot.Position.Z - root.Position.Z).Magnitude
		-- "Unreachable" = target is more than 12 studs above us
		-- AND more than 6 studs away horizontally (so we can't just stand below and jump)
		local unreachable = dy > UNREACHABLE_HEIGHT and horizDist > UNREACHABLE_HORIZ

		if unreachable then
			-- Don't MoveTo — player must navigate there manually
			return
		end
...
local function startDoubleJump()
	...
	doubleJumpInputConn = UserInputService.JumpRequest:Connect(function()
...
--  AUTO JUKE
-- =====================================================
--  Finds the nearest player (no highlight) who has a "Bomb" tool
--  equipped within AUTO_JUKE_DISTANCE studs.
--  Activates shiftlock-style camera lock and steers the local
--  character to juke the bomb hitbox: the script picks the
--  perpendicular / behind / front direction that moves the
--  local root AWAY from the bomb carrier's forward vector,
--  alternating strafes so the character weaves unpredictably.

local jukeShiftLockEnabled = false  -- tracks whether we forced shiftlock on

-- Shiftlock helper: enable/disable camera-relative movement (shiftlock effect)
-- Uses only safe APIs - no UserSettings/UserGameSettings which crash on many executors
local function setShiftLock(enabled)
	pcall(function()
		local cam = workspace.CurrentCamera
		if enabled and not jukeShiftLockEnabled then
			jukeShiftLockEnabled = true
			-- Lock mouse and switch to camera-relative movement via MouseBehavior
			UserInputService.MouseBehavior = Enum.MouseBehavior.LockCenter
		elseif not enabled and jukeShiftLockEnabled then
			jukeShiftLockEnabled = false
			UserInputService.MouseBehavior = Enum.MouseBehavior.Default
		end
	end)
end

-- Find nearest player who has Bomb equipped, no highlight, within range
local function findJukeTarget()
	local root = getRoot()
	...
	for _, p in ipairs(Players:GetPlayers()) do
		...
		local dist = (pRoot.Position - root.Position).Magnitude
		if dist <= AUTO_JUKE_DISTANCE and dist < bestDist then
			bestDist = dist; best = p; bestChar = char
		end
	end
	return best, bestChar
end

-- Juke phase cycles: we rotate through four offset directions
-- relative to the bomb carrier so the movement is unpredictable
local jukePhase     = 0
local jukePhaseTime = 0
local JUKE_PHASE_INTERVAL = 0.45  -- seconds per direction change

local function startAutoJuke()
	if autoJukeConn then return end
	jukePhase     = 0
	jukePhaseTime = tick()

	setShiftLock(true)

	autoJukeConn = RunService.Heartbeat:Connect(function()
		local root = getRoot()
		local hum  = getHum()
		if not root or not hum or hum.Health <= 0 then return end

		local carrier, carrierChar = findJukeTarget()
		if not carrier or not carrierChar then
			-- No valid juke target - stop moving
			hum:MoveTo(root.Position)
			return
		end

		local cRoot = carrierChar:FindFirstChild("HumanoidRootPart")
		if not cRoot then return end

		-- Phase switching for weave unpredictability
		local now = tick()
		if now - jukePhaseTime >= JUKE_PHASE_INTERVAL then
			jukePhase     = (jukePhase + 1) % 4
			jukePhaseTime = now
		end

		-- Direction from carrier to us (we want to move perpendicularly / behind)
		local toUs = (root.Position - cRoot.Position)
		local flatToUs = Vector3.new(toUs.X, 0, toUs.Z)
		local dist = flatToUs.Magnitude

		-- Build a safe perpendicular
		local forward
		if dist < 0.5 then
			-- Overlap: flee directly away from carrier's look
			forward = Vector3.new(cRoot.CFrame.LookVector.X, 0, cRoot.CFrame.LookVector.Z).Unit * -1
		else
			forward = flatToUs.Unit
		end

		-- Right vector (perpendicular in XZ plane)
		local right = Vector3.new(-forward.Z, 0, forward.X)

		-- Four juke offsets (stud magnitudes): right strafe, left strafe, behind, in-front
		local offsets = {
			right  * 8 + forward * 4,   -- phase 0: strafe right & stay away
			right  * -8 + forward * 4,  -- phase 1: strafe left  & stay away
			forward * 10,                -- phase 2: retreat straight back
			right  * 5 + forward * -3,  -- phase 3: cut across in front (risky but jukes hard)
		}

		local offset = offsets[jukePhase + 1]
		local target = root.Position + offset

		-- Keep on same Y (don't fly off platforms)
		target = Vector3.new(target.X, root.Position.Y, target.Z)

		hum:MoveTo(target)

		-- Jump if carrier is very close to dodge hitbox vertically
		local now2 = tick()
		if dist < 5 and now2 - lastJump >= JUMP_COOLDOWN then
			lastJump    = now2
			hum.Jump    = true
		end
	end)
...
local function stopAutoJuke()
	if autoJukeConn then autoJukeConn:Disconnect(); autoJukeConn = nil end
	setShiftLock(false)
	local h, r = getHum(), getRoot()
	if h and r then h:MoveTo(r.Position) end
end
...
local WIN_H = 300  -- main panel height
...
BtnClip.Position         = UDim2.new(0, 12, 0, 52)
...
Div2.Position         = UDim2.new(0, 12, 0, 97)
...
BombCard.Position         = UDim2.new(0, 12, 0, 106)
...
SettingsPanel.CanvasSize          = UDim2.new(0, 0, 0, 358)
...
-- -- Auto Juke toggle row ------------------------------
local AJDivider = Instance.new("Frame", SettingsPanel)
AJDivider.Size             = UDim2.new(1, -24, 0, 1)
AJDivider.Position         = UDim2.new(0, 12, 0, 253)
AJDivider.BackgroundColor3 = Color3.fromRGB(24, 24, 24)
AJDivider.BorderSizePixel  = 0

local AJRow = Instance.new("Frame", SettingsPanel)
AJRow.Size               = UDim2.new(1, -24, 0, 44)
AJRow.Position           = UDim2.new(0, 12, 0, 262)
AJRow.BackgroundColor3   = Color3.fromRGB(12, 12, 12)
AJRow.BorderSizePixel    = 0
Instance.new("UICorner", AJRow).CornerRadius = UDim.new(0, 9)
local AJRowStroke = Instance.new("UIStroke", AJRow)
AJRowStroke.Color        = Color3.fromRGB(40, 40, 40)
AJRowStroke.Thickness    = 1
AJRowStroke.Transparency = 0.25

local AJDot = Instance.new("Frame", AJRow)
AJDot.Size             = UDim2.new(0, 6, 0, 6)
AJDot.Position         = UDim2.new(0, 12, 0.5, -3)
AJDot.BackgroundColor3 = Color3.fromRGB(55, 55, 55)
AJDot.BorderSizePixel  = 0
AJDot.ZIndex           = 4
Instance.new("UICorner", AJDot).CornerRadius = UDim.new(1, 0)

local AJLabel = Instance.new("TextLabel", AJRow)
AJLabel.Size               = UDim2.new(1, -58, 0, 16)
AJLabel.Position           = UDim2.new(0, 26, 0, 5)
AJLabel.BackgroundTransparency = 1
AJLabel.Text               = "Auto Juke"
AJLabel.TextColor3         = Color3.fromRGB(175, 175, 175)
AJLabel.TextSize           = 10
AJLabel.Font               = Enum.Font.GothamBold
AJLabel.TextXAlignment     = Enum.TextXAlignment.Left

local AJSubLabel = Instance.new("TextLabel", AJRow)
AJSubLabel.Size               = UDim2.new(1, -58, 0, 11)
AJSubLabel.Position           = UDim2.new(0, 26, 1, -14)
AJSubLabel.BackgroundTransparency = 1
AJSubLabel.Text               = "Dodge bomb hitbox automatically"
AJSubLabel.TextColor3         = Color3.fromRGB(70, 70, 70)
AJSubLabel.TextSize           = 8
AJSubLabel.Font               = Enum.Font.Gotham
AJSubLabel.TextXAlignment     = Enum.TextXAlignment.Left

local AJToggle = Instance.new("TextButton", AJRow)
AJToggle.Size             = UDim2.new(0, 36, 0, 18)
AJToggle.Position         = UDim2.new(1, -44, 0.5, -9)
AJToggle.BackgroundColor3 = Color3.fromRGB(22, 22, 22)
AJToggle.Text             = ""
AJToggle.BorderSizePixel  = 0
AJToggle.AutoButtonColor  = false
AJToggle.ZIndex           = 6
Instance.new("UICorner", AJToggle).CornerRadius = UDim.new(1, 0)
local AJToggleStroke = Instance.new("UIStroke", AJToggle)
AJToggleStroke.Color        = Color3.fromRGB(60, 60, 60)
AJToggleStroke.Thickness    = 1
AJToggleStroke.Transparency = 0.2

local AJThumb = Instance.new("Frame", AJToggle)
AJThumb.Size             = UDim2.new(0, 12, 0, 12)
AJThumb.Position         = UDim2.new(0, 3, 0.5, -6)
AJThumb.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
AJThumb.BorderSizePixel  = 0
AJThumb.ZIndex           = 7
Instance.new("UICorner", AJThumb).CornerRadius = UDim.new(1, 0)

-- -- Auto Juke Distance textbox row --------------------
local AJDistDivider = Instance.new("Frame", SettingsPanel)
AJDistDivider.Size             = UDim2.new(1, -24, 0, 1)
AJDistDivider.Position         = UDim2.new(0, 12, 0, 314)
AJDistDivider.BackgroundColor3 = Color3.fromRGB(24, 24, 24)
AJDistDivider.BorderSizePixel  = 0

local AJDistRow = Instance.new("Frame", SettingsPanel)
AJDistRow.Size               = UDim2.new(1, -24, 0, 34)
AJDistRow.Position           = UDim2.new(0, 12, 0, 323)
AJDistRow.BackgroundColor3   = Color3.fromRGB(12, 12, 12)
AJDistRow.BorderSizePixel    = 0
Instance.new("UICorner", AJDistRow).CornerRadius = UDim.new(0, 9)
local AJDistRowStroke = Instance.new("UIStroke", AJDistRow)
AJDistRowStroke.Color        = Color3.fromRGB(40, 40, 40)
AJDistRowStroke.Thickness    = 1
AJDistRowStroke.Transparency = 0.25

local AJDistLabel = Instance.new("TextLabel", AJDistRow)
AJDistLabel.Size               = UDim2.new(0.55, 0, 1, 0)
AJDistLabel.Position           = UDim2.new(0, 12, 0, 0)
AJDistLabel.BackgroundTransparency = 1
AJDistLabel.Text               = "Auto Juke Distance"
AJDistLabel.TextColor3         = Color3.fromRGB(175, 175, 175)
AJDistLabel.TextSize           = 10
AJDistLabel.Font               = Enum.Font.GothamBold
AJDistLabel.TextXAlignment     = Enum.TextXAlignment.Left

local AJDistBox = Instance.new("TextBox", AJDistRow)
AJDistBox.Size               = UDim2.new(0, 48, 0, 22)
AJDistBox.Position           = UDim2.new(1, -58, 0.5, -11)
AJDistBox.BackgroundColor3   = Color3.fromRGB(20, 16, 32)
AJDistBox.Text               = "20"
AJDistBox.TextColor3         = Color3.fromRGB(210, 210, 210)
AJDistBox.TextSize           = 11
AJDistBox.Font               = Enum.Font.GothamBold
AJDistBox.ClearTextOnFocus   = false
AJDistBox.PlaceholderText    = "20"
AJDistBox.PlaceholderColor3  = Color3.fromRGB(80, 80, 80)
AJDistBox.BorderSizePixel    = 0
AJDistBox.ZIndex             = 6
AJDistBox.TextXAlignment     = Enum.TextXAlignment.Center
Instance.new("UICorner", AJDistBox).CornerRadius = UDim.new(0, 6)
local AJDistBoxStroke = Instance.new("UIStroke", AJDistBox)
AJDistBoxStroke.Color        = Color3.fromRGB(55, 55, 55)
AJDistBoxStroke.Thickness    = 1
AJDistBoxStroke.Transparency = 0.2

local AJDistUnit = Instance.new("TextLabel", AJDistRow)
AJDistUnit.Size               = UDim2.new(0, 16, 0, 14)
AJDistUnit.Position           = UDim2.new(1, -12, 0.5, -7)
AJDistUnit.BackgroundTransparency = 1
AJDistUnit.Text               = "st"
AJDistUnit.TextColor3         = Color3.fromRGB(80, 80, 80)
AJDistUnit.TextSize           = 9
AJDistUnit.Font               = Enum.Font.GothamBold
AJDistUnit.ZIndex             = 7
...
-- -- Auto Juke toggle handler --------------------------
local function applyAJToggle()
	if autoJuke then
		tw(AJToggle,      { BackgroundColor3 = Color3.fromRGB(40, 40, 40) }, 0.18)
		tw(AJThumb,       { Position = UDim2.new(1, -15, 0.5, -6), BackgroundColor3 = Color3.fromRGB(220, 220, 220) }, 0.2, Enum.EasingStyle.Back)
		tw(AJToggleStroke,{ Color = Color3.fromRGB(230, 230, 230) }, 0.18)
		tw(AJDot,         { BackgroundColor3 = Color3.fromRGB(230, 230, 230) }, 0.2)
		tw(AJLabel,       { TextColor3 = Color3.fromRGB(220, 220, 220) }, 0.2)
		tw(AJRowStroke,   { Color = Color3.fromRGB(50, 50, 50), Transparency = 0.08 }, 0.2)
		startAutoJuke()
	else
		tw(AJToggle,      { BackgroundColor3 = Color3.fromRGB(22, 22, 22) }, 0.18)
		tw(AJThumb,       { Position = UDim2.new(0, 3, 0.5, -6), BackgroundColor3 = Color3.fromRGB(100, 100, 100) }, 0.2, Enum.EasingStyle.Back)
		tw(AJToggleStroke,{ Color = Color3.fromRGB(60, 60, 60) }, 0.18)
		tw(AJDot,         { BackgroundColor3 = Color3.fromRGB(55, 55, 55) }, 0.2)
		tw(AJLabel,       { TextColor3 = Color3.fromRGB(175, 175, 175) }, 0.2)
		tw(AJRowStroke,   { Color = Color3.fromRGB(40, 40, 40), Transparency = 0.25 }, 0.2)
		stopAutoJuke()
	end
end

AJToggle.MouseButton1Click:Connect(function()
	autoJuke = not autoJuke
	applyAJToggle()
end)
AJToggle.MouseEnter:Connect(function()
	tw(AJRow, { BackgroundColor3 = Color3.fromRGB(20, 20, 20) }, 0.12)
end)
AJToggle.MouseLeave:Connect(function()
	tw(AJRow, { BackgroundColor3 = Color3.fromRGB(12, 12, 12) }, 0.12)
end)

AJDistBox.FocusLost:Connect(function()
	local val = tonumber(AJDistBox.Text)
	if val then
		val = math.clamp(math.floor(val), 1, 200)
		AUTO_JUKE_DISTANCE = val
		AJDistBox.Text = tostring(AUTO_JUKE_DISTANCE)
		tw(AJDistBoxStroke, { Color = Color3.fromRGB(180, 180, 180) }, 0.12)
		task.delay(0.6, function()
			tw(AJDistBoxStroke, { Color = Color3.fromRGB(55, 55, 55) }, 0.3)
		end)
	else
		AJDistBox.Text = tostring(AUTO_JUKE_DISTANCE)
		tw(AJDistBoxStroke, { Color = Color3.fromRGB(200, 60, 60) }, 0.12)
		task.delay(0.6, function()
			tw(AJDistBoxStroke, { Color = Color3.fromRGB(55, 55, 55) }, 0.3)
		end)
	end
